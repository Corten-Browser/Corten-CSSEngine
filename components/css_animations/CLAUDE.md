...
You are building ONLY the `css_animations` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_animations/`
...
### Commit Pattern
```bash
git commit -m "[css_animations] test: add color parsing tests"
git commit -m "[css_animations] feat: implement color parsing (makes tests pass)"
git commit -m "[css_animations] refactor: extract hex validation logic"
```
...
git add components/css_animations/

# Commit with prefix
git commit -m "[css_animations] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_animations" "feat: implement selector matching"
```
...
Your component implements section(s) related to CSS animations and transitions.
