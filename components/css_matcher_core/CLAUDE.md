...
You are building ONLY the `css_matcher_core` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_matcher_core/`
...
### Commit Pattern
```bash
git commit -m "[css_matcher_core] test: add color parsing tests"
git commit -m "[css_matcher_core] feat: implement color parsing (makes tests pass)"
git commit -m "[css_matcher_core] refactor: extract hex validation logic"
```
...
git add components/css_matcher_core/

# Commit with prefix
git commit -m "[css_matcher_core] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_matcher_core" "feat: implement selector matching"
```
...
Your component implements section(s) related to Core selector matching.
