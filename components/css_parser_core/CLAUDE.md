...
**Type**: core (10000 lines estimated)
**Tech Stack**: Rust 2021, cssparser
**Project**: Corten CSS Engine v0.1.0

You are building ONLY the `css_parser_core` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_parser_core/`
...
### Commit Pattern
```bash
git commit -m "[css_parser_core] test: add color parsing tests"
git commit -m "[css_parser_core] feat: implement color parsing (makes tests pass)"
git commit -m "[css_parser_core] refactor: extract hex validation logic"
```
...
git add components/css_parser_core/

# Commit with prefix
git commit -m "[css_parser_core] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_parser_core" "feat: implement selector matching"
```
...
Your component implements section(s) related to Core CSS parsing.
