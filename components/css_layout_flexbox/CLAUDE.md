...
You are building ONLY the `css_layout_flexbox` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_layout_flexbox/`
...
### Commit Pattern
```bash
git commit -m "[css_layout_flexbox] test: add color parsing tests"
git commit -m "[css_layout_flexbox] feat: implement color parsing (makes tests pass)"
git commit -m "[css_layout_flexbox] refactor: extract hex validation logic"
```
...
git add components/css_layout_flexbox/

# Commit with prefix
git commit -m "[css_layout_flexbox] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_layout_flexbox" "feat: implement selector matching"
```
...
Your component implements section(s) related to Flexbox layout properties.
