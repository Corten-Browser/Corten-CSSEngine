...
**Type**: base (8000 lines estimated)
**Tech Stack**: Rust 2021
**Project**: Corten CSS Engine v0.1.0

You are building ONLY the `css_types` component.
...
### Purpose
Shared type definitions for CSS engine

### Dependencies
None (base component)

### Boundaries
- ✅ Work ONLY in `components/css_types/`
...
### Commit Pattern
```bash
git commit -m "[css_types] test: add color parsing tests"
git commit -m "[css_types] feat: implement color parsing (makes tests pass)"
git commit -m "[css_types] refactor: extract hex validation logic"
```
...
git add components/css_types/

# Commit with prefix
git commit -m "[css_types] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_types" "feat: implement selector matching"
```
...
Your component implements section(s) related to Shared type definitions for CSS engine.
