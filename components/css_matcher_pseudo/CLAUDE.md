...
**Type**: core (4000 lines estimated)
**Tech Stack**: Rust 2021, selectors
**Project**: Corten CSS Engine v0.1.0

You are building ONLY the `css_matcher_pseudo` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_matcher_pseudo/`
...
### Commit Pattern
```bash
git commit -m "[css_matcher_pseudo] test: add color parsing tests"
git commit -m "[css_matcher_pseudo] feat: implement color parsing (makes tests pass)"
git commit -m "[css_matcher_pseudo] refactor: extract hex validation logic"
```
...
git add components/css_matcher_pseudo/

# Commit with prefix
git commit -m "[css_matcher_pseudo] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_matcher_pseudo" "feat: implement selector matching"
```
...
Your component implements section(s) related to Pseudo-class/element matching.
