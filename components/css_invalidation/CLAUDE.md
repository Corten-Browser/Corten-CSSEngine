...
You are building ONLY the `css_invalidation` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_invalidation/`
...
### Commit Pattern
```bash
git commit -m "[css_invalidation] test: add color parsing tests"
git commit -m "[css_invalidation] feat: implement color parsing (makes tests pass)"
git commit -m "[css_invalidation] refactor: extract hex validation logic"
```
...
git add components/css_invalidation/

# Commit with prefix
git commit -m "[css_invalidation] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_invalidation" "feat: implement selector matching"
```
...
Your component implements section(s) related to Incremental restyle.
