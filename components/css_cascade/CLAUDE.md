...
You are building ONLY the `css_cascade` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_cascade/`
...
### Commit Pattern
```bash
git commit -m "[css_cascade] test: add color parsing tests"
git commit -m "[css_cascade] feat: implement color parsing (makes tests pass)"
git commit -m "[css_cascade] refactor: extract hex validation logic"
```
...
git add components/css_cascade/

# Commit with prefix
git commit -m "[css_cascade] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_cascade" "feat: implement selector matching"
```
...
Your component implements section(s) related to CSS cascade algorithm.
