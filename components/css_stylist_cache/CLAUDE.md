...
**Type**: feature (6000 lines estimated)
**Tech Stack**: Rust 2021, lru
**Project**: Corten CSS Engine v0.1.0

You are building ONLY the `css_stylist_cache` component.
...
### Boundaries
- ✅ Work ONLY in `components/css_stylist_cache/`
...
### Commit Pattern
```bash
git commit -m "[css_stylist_cache] test: add color parsing tests"
git commit -m "[css_stylist_cache] feat: implement color parsing (makes tests pass)"
git commit -m "[css_stylist_cache] refactor: extract hex validation logic"
```
...
git add components/css_stylist_cache/

# Commit with prefix
git commit -m "[css_stylist_cache] feat: implement selector matching"

# Or use retry wrapper
python ../../orchestration/git_retry.py "css_stylist_cache" "feat: implement selector matching"
```
...
Your component implements section(s) related to Style sharing and caching.
